...
class TEOSErrorErrorLocalFree : public boost::noncopyable {
public:
    explicit __fastcall TEOSErrorErrorLocalFree( PVOID Buffer )
        : Buffer_( Buffer ) {}
    __fastcall ~TEOSErrorErrorLocalFree() { LocalFree( Buffer_ ); }
private:
    PVOID Buffer_;
};
...
String FormatOSErrorMessage( DWORD ErrCode )
{
    ...
    TEOSErrorErrorLocalFree LocalFreeObj( Buffer );
...
typedef Gdiplus::Graphics GG;

void DrawImage1( Gdiplus::Graphics& g, Gdiplus::Image& Img, Gdiplus::RectF const & DstRect );

//void DrawImage( GdiPlus::Graphics& g, Image& Img, GdiPlus::RectF const & DstRect,
